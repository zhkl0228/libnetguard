...
public class TCPPacketFactory {
	...
	private static final Logger log = LoggerFactory.getLogger(TCPPacketFactory.class);
	...
	public static Packet createSynAckPacketData(IPv4Header ip, TCPHeader tcp){
		...
		tcpheader.setSequenceNumber(seqNumber);
		log.debug("Set Initial Sequence number: {}", seqNumber);
	 ...
	 */
	public static TCPHeader createTCPHeader(ByteBuffer stream) throws PacketHeaderException {
...
//	private static final int SELECTIVE_ACK = 5;
	...
	private static void handleTcpOptions(TCPHeader header, ByteBuffer packet, int optionsSize) {
		int index = 0;
	...
	private static void skipRemainingOptions(ByteBuffer packet, int size) {
