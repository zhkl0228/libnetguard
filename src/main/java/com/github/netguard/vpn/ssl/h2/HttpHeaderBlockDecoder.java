...
public final class HttpHeaderBlockDecoder {

    private static final Logger log = LoggerFactory.getLogger(HttpHeaderBlockDecoder.class);
    ...
    public void decode(ByteBuf headerBlock, final HttpHeaderBlockFrame frame) throws IOException {
        ...
        if (cumulation == null) {
            decodeInputStream(new ByteBufInputStream(headerBlock), headerListener);
            log.debug("decode headerBlock={}", headerBlock);
            ...
            cumulation.writeBytes(headerBlock);
            decodeInputStream(new ByteBufInputStream(cumulation), headerListener);
            log.debug("decode cumulation={}", cumulation);
