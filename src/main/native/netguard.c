...
static jmethodID midNotifyPacket = NULL;
...
void notify_packet(const struct arguments *args, const uint8_t *buffer, size_t length, jint uid) {
    ...
    const char *signature = "(I[B)V";
    if (midNotifyPacket == NULL) {
        midNotifyPacket = jniGetMethodID(args->env, clsService, "notifyPacket", signature);
    }

    jbyteArray packet = (*args->env)->NewByteArray(args->env, (jsize) length);
    (*args->env)->SetByteArrayRegion(args->env, packet, 0, (jsize) length, (const jbyte *) buffer);

    (*args->env)->CallVoidMethod(args->env, args->instance, midNotifyPacket, uid, packet);
    jniCheckException(args->env);

    (*args->env)->DeleteLocalRef(args->env, clsService);
    (*args->env)->DeleteLocalRef(args->env, packet);
    ...
    mselapsed = (end.tv_sec - start.tv_sec) * 1000.0 +
                (end.tv_usec - start.tv_usec) / 1000.0;
    if (mselapsed > PROFILE_JNI)
        log_android(ANDROID_LOG_WARN, "notify_packet %f", mselapsed);
#endif
}
